...
HttpRequestParser.prototype.parse = function(request) {
  ...
  var cookie = parseCookieRow(requestRows.cookie);
...
function parseRequestForRows(request) {
  ...
  var cookieIndex = _.findIndex(headersRows, function (row) {
    return _s.startsWith(row, 'Cookie:');
  });
  var cookieRow;
  if (cookieIndex !== -1) {
    cookieRow = headersRows[cookieIndex];
    headersRows.splice(cookieIndex, 1);
  }
...
function parseCookieRow(row) {
  ...
  var cookie = _s.words(nameAndValues[1], ';');

  return _(cookie)
    .map(function (cook) {
      var cookieAndValue = _s.words(cook, '=');
...
function getContentTypeAndBoundary(headers) {
  ...
  if (!contentTypeHeader.values || contentTypeHeader.values.length === 0) {
    return null;
  }
  
  var contentTypeValue = contentTypeHeader.values[0];  
  var boundary = null;
